import { AnalysisSignal, EvidenceExtractionResult, EvidenceSignal, StressLevel, StressResult, StructuredExtractionResult, ToddlerCategory } from './types';
...
const STRUCTURED_POSITIVE_WEIGHTS: Record<string, { category: ToddlerCategory; delta: number }> = {
  high_chairs:         { category: 'high chairs available', delta: 2 },
  kids_menu:           { category: 'kids menu', delta: 2 },
  pram_space:          { category: 'pram or buggy space', delta: 1 },
  changing_table:      { category: 'family friendly atmosphere', delta: 1 },
  staff_child_friendly:{ category: 'welcoming staff toward children', delta: 1 },
  noise_tolerant:      { category: 'family friendly atmosphere', delta: 1 },
};
...
export function scoreStructuredExtraction(input: StructuredExtractionResult): ScoringResult {
  ...
  for (const [key, weight] of Object.entries(STRUCTURED_POSITIVE_WEIGHTS)) {
    ...
    if (value === true) {
      positive_signals.push({ category: weight.category, evidence: '' });
      score += weight.delta;
      signalCount++;
    } else if (value === false) {
      negative_signals.push({ category: weight.category, evidence: '' });
      score -= weight.delta;
      signalCount++;
    }
  }
